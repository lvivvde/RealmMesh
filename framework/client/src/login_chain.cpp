...
LoginChain::Step LoginChain::take_ticket(TimePoint deadline) {
    const auto result =
...
LoginChain::Step LoginChain::poll_until_admitted(TimePoint deadline) {
...
LoginChain::Step LoginChain::connect_gateway_and_handoff(TimePoint deadline) {
    for (;;) {
        if (Clock::now() >= deadline) {
        ...
        if (connected.ok) {
            ...
            if (attached.ok) {
                ...
                if (attached.credential_expired) {
                    return Step::RetakeTicket;
                }
...
LoginChain::Step LoginChain::redeem_realm(TimePoint deadline) {
...
LoginChainResult LoginChain::run(std::string_view account,
    ...
    for (;;) {
        ...
        if (take_ticket(deadline) == Step::Failed) {
            set_stage(LoginStage::Idle);
            return finish(LoginStage::Idle);
        }
        ...
        if (admitted == Step::Failed) {
            set_stage(LoginStage::Idle);
            return finish(LoginStage::Idle);
        }
        if (admitted == Step::RetakeTicket) {
            continue;
        }
        ...
        for (;;) {
            ...
            if (gateway == Step::Failed) {
                set_stage(LoginStage::Idle);
                return finish(LoginStage::Idle);
            }
            if (gateway == Step::RetakeTicket) {
                retake_ticket = true;
                break;
            }

            const auto realm = redeem_realm(deadline);
            if (realm == Step::Advanced) {
                return finish(LoginStage::InGame);
            }
            if (realm == Step::RetakeTicket) {
                retake_ticket = true;
                break;
            }
            if (Clock::now() >= deadline) {
