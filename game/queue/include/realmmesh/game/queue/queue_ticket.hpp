...
/// 区分凭据类型。
inline constexpr std::string_view queue_ticket_issuer = "realmmesh/queue";

/// 排队号牌的 claims 集(ADR-0006):号值 + admitted。schema 受控——无
/// sub/jti/aud(号牌不单次消费,位次查询复用,断线凭它找回位次)。
struct QueueTicketClaims {
...
class QueueTicketCodec final {
public:
    QueueTicketCodec(common::Ed25519Seed seed, std::string kid);

    /// 签发紧缩三段式 JWS;前提:claims 由本方签发路径构造
    /// (number ≥ 1、expires_at 不早于 issued_at)。
    [[nodiscard]] std::string issue(const QueueTicketClaims& claims) const;

    /// 验签在 claims 语义之前;头/载荷逐键核对,签名不合法即整体拒绝。
    /// number 为 0 视为非法(先例同 IdentityTokenCodec 的 account_id)。
    [[nodiscard]] std::optional<QueueTicketClaims> validate(
