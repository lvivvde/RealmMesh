...
constexpr std::byte ticket_version{1};
constexpr std::size_t signed_size = 1 + 1 + session_ticket_id_size + 8 + 4 + 8 + 8;
constexpr std::size_t tag_size = crypto_auth_hmacsha256_BYTES;
constexpr std::size_t ticket_size = signed_size + tag_size;
...
std::vector<std::byte> SessionTicketCodec::issue(
    ...
    std::vector<std::byte> ticket(ticket_size);
...
std::optional<SessionTicketClaims> SessionTicketCodec::validate(
    ...
    std::chrono::system_clock::time_point now) const {
    if (ticket.size() != ticket_size || ticket[0] != ticket_version ||
